...
def _instruction_block(
    ...
    body = (
        ...
        f"It was generated by [rtt](https://github.com/yttrium400/reducethemtokens-rtt-) "
        ...
        f"**Read `{context_path}` once at the start of each session** to orient yourself "
        f"before opening individual files. Use it to:\n\n"
        f"- Understand what exists in the codebase and where things are defined\n"
        f"- Identify which specific files are relevant to the current task\n"
        f"- Answer structural questions (signatures, class members, imports) "
        f"without scanning multiple files\n\n"
        f"After reading the index, work as you normally would: read specific source "
